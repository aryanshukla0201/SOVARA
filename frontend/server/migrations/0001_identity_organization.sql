...
CREATE TABLE IF NOT EXISTS organization_memberships (
  ...
  role VARCHAR(40) NOT NULL CHECK (role IN ('ORG_ADMIN', 'DIVISION_MANAGER', 'DEPARTMENT_MANAGER', 'TEAM_LEAD', 'EMPLOYEE')),
...
SELECT r.id, p.id FROM roles r CROSS JOIN permissions p WHERE r.code = 'ORG_ADMIN'
ON CONFLICT DO NOTHING;

INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN (
  'user.read', 'organization.read', 'organization.members.read', 'division.read', 'division.manage',
  'department.read', 'department.manage', 'team.read', 'team.manage', 'meeting.read', 'meeting.create',
  'meeting.update', 'meeting.join', 'meeting.manage_participants', 'document.read', 'document.create',
...
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN (
  'user.read', 'organization.read', 'organization.members.read', 'department.read', 'department.manage',
  'team.read', 'team.manage', 'meeting.read', 'meeting.create', 'meeting.update', 'meeting.join',
  'meeting.manage_participants', 'document.read', 'document.create', 'document.update', 'artifact.read',
  'artifact.create'
...
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN (
  'user.read', 'organization.read', 'organization.members.read', 'team.read', 'team.manage', 'meeting.read',
  'meeting.create', 'meeting.join', 'meeting.manage_participants', 'document.read', 'document.create',
...
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN (
  'organization.read', 'organization.members.read', 'team.read', 'meeting.read', 'meeting.join',
  'document.read', 'artifact.read'
